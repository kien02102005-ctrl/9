...
<body>
    ...
    <script>
        ...
        function adminChangeUserKPIType(newType) {
            ...
            const targetUserInfo = registeredUsers.find(u => u.username === kpiTargetUser);
            if (targetUserInfo) {
                targetUserInfo.kpiType = newType;
                db.ref(`users/${kpiTargetUser}/kpiType`).set(newType, (err) => {
                    if (!err) {
                        alert(`Đã cập nhật loại Bảng KPI của ${kpiTargetUser} thành ${getKPITypeName(newType)}`);
                        listenRealtimeKPI();
                    }
                });
            }
        ...
        function getKPITypeName(type) {
            if (type === 'leader') return 'Bảng KPI Lãnh Đạo';
            if (type === 'cleaner') return 'Bảng KPI Lao Công';
            return 'Bảng KPI Cán Bộ / Nhân Viên';
        }
        ...
        function renderUserManagementTable() {
            ...
            registeredUsers.forEach((u, index) => {
                const tr = document.createElement('tr');
                const userKpiType = u.kpiType || 'staff';
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <select onchange="updateUserKPITypeInTable('${u.username}', this.value)" style="padding:4px 8px; border-radius:4px; border:1px solid #cbd5e1; font-weight:bold; color:#1e40af;">
                            <option value="staff" ${userKpiType === 'staff' ? 'selected' : ''}>Bảng Cán bộ / Nhân viên</option>
                            <option value="leader" ${userKpiType === 'leader' ? 'selected' : ''}>Bảng Lãnh đạo</option>
                            <option value="cleaner" ${userKpiType === 'cleaner' ? 'selected' : ''}>Bảng Lao công</option>
                        </select>
                    </td>
                    <td>
                        ${u.username !== ADMIN_USERNAME ? `
                            <button class="btn-sm btn-delete" onclick="deleteUser('${u.username}')"><i class="fa-solid fa-trash"></i> Xóa tài khoản</button>
                        ` : '<span style="color:#94a3b8; font-size:12px;">Hệ thống</span>'}
                    </td>
                `;
                tbody.appendChild(tr);
            });
        ...
        function updateUserKPITypeInTable(username, newType) {
            db.ref(`users/${username}/kpiType`).set(newType, (err) => {
                if (!err) {
                    alert(`Đã đổi loại bảng KPI cho tài khoản ${username}!`);
        ...
        function deleteUser(username) {
            if (confirm(`Bạn có chắc muốn xóa vĩnh viễn tài khoản ${username}?`)) {
        ...
        function switchTab(tabId, navElement) {
            ...
            const titleMap = {
                ...
                'kpi': 'Đánh Giá Điểm KPI',
                'admin-users': 'Quản Lý Tài Khoản Hệ Thống',
                'box-chat': 'Box Chat Realtime'
            };
            document.getElementById('page-title').innerText = titleMap[tabId] || 'Workspace';
        ...
        function renderDashboard() {
            let evalTasks = [];
            if (isAdmin) {
                Object.values(allUsersTasksMap).forEach(userTasks => {
                    if (userTasks && typeof userTasks === 'object') {
                        evalTasks = evalTasks.concat(Object.values(userTasks));
                    }
                });
            } else {
                evalTasks = tasks;
            ...
            evalTasks.forEach(t => {
                if (t.status === 'Chưa làm') todo++;
                else if (t.status === 'Đang làm') doing++;
                else if (t.status === 'Hoàn thành') done++;

                if (t.priority === 'Thấp') low++;
                else if (t.priority === 'Bình thường') normal++;
                else if (t.priority === 'Cao') high++;
            });
            ...
            priorityChartInstance = new Chart(ctxPriority, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['#94a3b8', '#0284c7', '#ef4444']
                    }]
                },
        ...
        function renderTaskList() {
            ...
            if (tasks.length === 0) {
                tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; color:#94a3b8; padding:20px;">Chưa có công việc nào.</td></tr>`;
                return;
            }

            tasks.forEach((t, index) => {
                const isOverdue = isTaskOverdue(t.date, t.status);
                const tr = document.createElement('tr');
                if (isOverdue) tr.classList.add('row-overdue');

                const statusClass = t.status === 'Đang làm' ? 'status-doing' : t.status === 'Hoàn thành' ? 'status-done' : 'status-todo';

                let filesHTML = '';
                if (t.files && t.files.length > 0) {
                    filesHTML = t.files.map((f, fIdx) => `
                        <div class="file-tag">
                            <i class="fa-solid fa-paperclip"></i>
                            ...
                            <i class="fa-solid fa-xmark" style="cursor:pointer; color:#ef4444; margin-left:4px;" onclick="deleteTaskFile(${index}, ${fIdx})"></i>
                        </div>
                    `).join('');
                }
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <span contenteditable="true" onblur="updateTaskName(${index}, this.innerText)">${t.name}</span>
                        ${isOverdue ? '<span class="badge-overdue"><i class="fa-solid fa-triangle-exclamation"></i> QUÁ HẠN</span>' : ''}
                    </td>
                    <td><span style="font-weight:600; color:#1e40af;">${t.user || targetUser}</span></td>
                    <td>
                        <select onchange="updateTaskPriority(${index}, this.value)" style="border:none; background:transparent; font-weight:bold;">
                            <option value="Bình thường" ${t.priority === 'Bình thường' ? 'selected' : ''}>Bình thường</option>
                            <option value="Cao" ${t.priority === 'Cao' ? 'selected' : ''}>Cao</option>
                            <option value="Thấp" ${t.priority === 'Thấp' ? 'selected' : ''}>Thấp</option>
                        </select>
                    </td>
                    <td>
                        <select class="status-tag ${statusClass}" onchange="updateTaskStatus(${index}, this.value)">
                    ...
                    <td>
                        <input type="date" class="inline-date-picker" value="${convertToISODate(t.date)}" onchange="updateTaskDate(${index}, this.value)" />
                    </td>
                    <td>
                        ${filesHTML}
                        <div>
                            <input type="file" id="file-input-${index}" style="display:none;" onchange="attachTaskFile(${index}, this)" />
                            <button class="btn-sm btn-attach" onclick="document.getElementById('file-input-${index}').click()"><i class="fa-solid fa-paperclip"></i> Thêm file</button>
                        </div>
                    </td>
                    <td>
        ...
        function addNewTask() {
            ...
            const dateVal = document.getElementById('newTaskDueDate').value;

            if (!name) { alert('Vui lòng nhập tên công việc!'); return; }

            const newId = 'T' + String(tasks.length + 1).padStart(3, '0');
            const displayDate = convertToDisplayDate(dateVal);
        ...
        function updateTaskName(index, name) { tasks[index].name = name.trim(); saveUserData(); }
        function updateTaskPriority(index, priority) { tasks[index].priority = priority; saveUserData(); }
        function updateTaskStatus(index, status) { tasks[index].status = status; saveUserData(); }
        function updateTaskDate(index, isoDateStr) { tasks[index].date = convertToDisplayDate(isoDateStr); saveUserData(); }
        function deleteTask(index) { if (confirm('Bạn có chắc muốn xóa công việc này?')) { tasks.splice(index, 1); saveUserData(); } }

        function attachTaskFile(taskIndex, inputEl) {
            const file = inputEl.files[0];
            if (!file) return;

            if (file.size > 5 * 1024 * 1024) {
                alert('Tệp quá lớn (Vượt quá 5MB). Vui lòng chọn tệp nhỏ hơn!');
                inputEl.value = '';
                return;
            }
        ...
        function deleteTaskFile(taskIndex, fileIndex) {
            if (confirm('Bạn có chắc muốn xóa tệp đính kèm này?')) {
                tasks[taskIndex].files.splice(fileIndex, 1);
                saveUserData();
            }
        }
        ...
        function renderKanban() {
            ...
            cardsTodo.innerHTML = ''; cardsDoing.innerHTML = ''; cardsDone.innerHTML = '';
            ...
            tasks.forEach((t, index) => {
                ...
                card.className = `kanban-card ${isOverdue ? 'card-overdue' : ''}`;
                card.draggable = true;
                card.ondragstart = (e) => e.dataTransfer.setData('text/plain', index);

                card.innerHTML = `
                    <div class="id">${t.id} ${isOverdue ? '<span class="badge-overdue" style="font-size:9px;"><i class="fa-solid fa-triangle-exclamation"></i> QUÁ HẠN</span>' : ''}</div>
                    <div class="title">${t.name}</div>
                    <div class="meta">
                        <span><i class="fa-regular fa-clock"></i> ${t.date || 'Chưa có'}</span>
                        <span style="font-weight:bold; color: ${t.priority === 'Cao' ? '#ef4444' : t.priority === 'Thấp' ? '#94a3b8' : '#0284c7'};">${t.priority}</span>
                    </div>
                `;
        ...
        function renderKPITable() {
            const container = document.getElementById('kpi-sections-wrapper');
            if (!container) return;
            container.innerHTML = '';
            ...
            const nameDisplay = document.getElementById('kpi-target-name-display');
            if (nameDisplay) nameDisplay.innerText = kpiTargetUser;

            const badgeDisplay = document.getElementById('kpi-type-badge-display');
            if (badgeDisplay) {
                badgeDisplay.innerText = getKPITypeName(userKpiType);
                badgeDisplay.className = `kpi-badge-type type-${userKpiType}`;
            }

            const lastSavedEl = document.getElementById('kpi-last-time-saved');
            if (lastSavedEl) lastSavedEl.innerText = lastKpiTimestamp;
            ...
            kpiDataList.forEach(sub => {
                ...
                sectionGroups[sec].push(sub);
            });

            Object.keys(sectionGroups).forEach(secKey => {
                const subList = sectionGroups[secKey];
                const secTitle = sectionTitles[secKey] || `MỤC ${secKey}`;
                ...
                totalSelfSum += secSelfSum;
                totalAdminSum += secAdminSum;

                const secCard = document.createElement('div');
                secCard.className = 'card';
                secCard.style.padding = '0';
                secCard.style.overflow = 'hidden';
                secCard.style.marginBottom = '20px';

                secCard.innerHTML = `
                    <div class="kpi-section-title">
                        <span><i class="fa-solid fa-layer-group"></i> ${secKey}. ${secTitle.toUpperCase()}</span>
                        <div>
                            ${isAdmin ? `
                                <button class="btn-sm btn-edit" onclick="editMainSectionTitle('${secKey}')"><i class="fa-solid fa-pen"></i> Đổi tên</button>
                                <button class="btn-sm btn-attach" onclick="addSubSection('${secKey}')"><i class="fa-solid fa-plus"></i> Thêm Nhóm Con</button>
                            ` : ''}
                        </div>
                    </div>
                    <div class="table-container">
                        <table class="kpi-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 250px;">Tiêu Chí Chi Tiết / Yêu Cầu</th>
                                    ...
                                    ${isAdmin ? '<th style="width: 90px; text-align: center;">Thao tác</th>' : ''}
                                </tr>
                            </thead>
                            <tbody id="tbody-section-${secKey}"></tbody>
                        </table>
                    </div>
                `;

                container.appendChild(secCard);
                const tbody = document.getElementById(`tbody-section-${secKey}`);
                ...
                subList.forEach((sub, subIdx) => {
                    ...
                    if (sub.items) {
                        sub.items.forEach(it => {
                            subSelfTotal += parseFloatStrict(it.selfScore);
                            subAdminTotal += parseFloatStrict(it.adminScore);
                        });
                    }
                    ...
                    trSub.innerHTML = `
                        ...
                        <td style="text-align: center; color: #1e40af;"><strong>${subSelfTotal.toFixed(1)}</strong></td>
                        <td style="text-align: center; color: #059669;"><strong>${subAdminTotal.toFixed(1)}</strong></td>
                        ${isAdmin ? `
                            <td style="text-align: center;">
                                <button class="btn-sm btn-attach" title="Thêm chỉ tiêu con" onclick="addItemToSubSection('${sub.id}')"><i class="fa-solid fa-plus"></i></button>
                                <button class="btn-sm btn-delete" title="Xóa nhóm con" onclick="deleteSubSection('${sub.id}')"><i class="fa-solid fa-trash"></i></button>
                            </td>
                        ` : ''}
                    `;
                    ...
                    if (sub.items) {
                        sub.items.forEach((item, itemIdx) => {
                            ...
                            trItem.innerHTML = `
                                ...
                                <td>
                                    <span ${isAdmin ? 'contenteditable="true" onblur="updateKPIItemField(\'' + item.id + '\', \'title\', this.innerText)"' : ''}>${item.title}</span>
                                </td>
                                <td>
                                    <span ${isAdmin ? 'contenteditable="true" onblur="updateKPIItemField(\'' + item.id + '\', \'criteria\', this.innerText)"' : ''} style="font-size: 13px; color: #475569;">${item.criteria || ''}</span>
                                </td>
                                <td style="text-align: center;">
                                    ${isAdmin ? `<input type="number" step="0.5" value="${item.maxScore}" onchange="updateKPIItemField('${item.id}', 'maxScore', this.value)" />` : `<strong>${item.maxScore}</strong>`}
                                </td>
                                <td style="text-align: center;">
                                    <input type="number" step="0.5" value="${item.selfScore}" onchange="updateKPIItemScore('${item.id}', 'selfScore', this.value)" />
                                </td>
                                <td style="text-align: center;">
                                    ${isAdmin ? `<input type="number" step="0.5" value="${item.adminScore}" onchange="updateKPIItemScore('${item.id}', 'adminScore', this.value)" />` : `<strong style="color:#059669;">${item.adminScore}</strong>`}
                                </td>
                                ${isAdmin ? `
                                    <td style="text-align: center;">
                                        <button class="btn-sm btn-delete" onclick="deleteKPIItem('${item.id}')"><i class="fa-solid fa-trash"></i></button>
                                    </td>
                                ` : ''}
                            `;
            ...
            document.getElementById('kpi-total-self').innerText = totalSelfSum.toFixed(1);
            document.getElementById('kpi-total-max').innerText = '100';
            document.getElementById('kpi-total-admin').innerText = totalAdminSum.toFixed(1);

            updateRankingTable(totalAdminSum);
        }

        function updateRankingTable(finalScore) {
            for (let i = 1; i <= 4; i++) {
                const el = document.getElementById(`rank-check-${i}`);
                if (el) el.innerHTML = '';
            }
            ...
            if (!isKpiSavedForCurrentMonth && lastKpiTimestamp === 'Chưa có dữ liệu') {
                badge.innerText = 'Chưa xếp loại';
        ...
        function updateKPIItemScore(itemId, scoreType, val) {
            const floatVal = parseFloatStrict(val);
            kpiDataList.forEach(sub => {
                if (sub.items) {
                    sub.items.forEach(item => {
                        if (item.id === itemId) {
                            if (floatVal > item.maxScore) {
                                alert(`Điểm nhập (${floatVal}) không được vượt quá Điểm tối đa (${item.maxScore})!`);
                                renderKPITable();
                                return;
                            }
                            item[scoreType] = floatVal;
                        }
                    });
                }
            });
        ...
        function updateKPIItemField(itemId, field, val) {
            kpiDataList.forEach(sub => {
                if (sub.items) {
                    sub.items.forEach(item => {
                        if (item.id === itemId) {
                            if (field === 'maxScore') item[field] = parseFloatStrict(val);
        ...
        function saveKPIRatingByAdmin() {
            ...
            lastKpiTimestamp = `${now} (Bởi Admin ${currentUser})`;
            ...
            db.ref(`kpiTemplates/${userKpiType}`).set({
                sectionMaxScores: sectionMaxScores,
                sectionTitles: sectionTitles,
                kpiDataList: kpiDataList
            });

            alert(`Admin đã lưu kết quả đánh giá KPI và đồng bộ khung Mẫu cho tài khoản ${kpiTargetUser} thành công!`);
        }

        function saveKPIRatingByUser() {
            const now = new Date().toLocaleString('vi-VN');
            lastKpiTimestamp = `${now} (Tự chấm bởi ${currentUser})`;

            saveKPIRatingStorage(lastKpiTimestamp);
            alert(`Bạn đã lưu điểm tự chấm KPI cho tháng thành công!`);
        }

        function addMainSection() {
            if (!isAdmin) return;
            const secKey = prompt('Nhập chữ cái mã Mục Lớn mới (ví dụ: D, E, F...):');
            if (!secKey) return;
            const cleanKey = secKey.trim().toUpperCase();
            
            const title = prompt(`Nhập tiêu đề cho Mục Lớn ${cleanKey}:`);
            if (!title) return;

            sectionTitles[cleanKey] = title.trim();
            sectionMaxScores[cleanKey] = 20;

            addSubSection(cleanKey);
        }

        function editMainSectionTitle(secKey) {
            if (!isAdmin) return;
            const newTitle = prompt(`Nhập tiêu đề mới cho Mục ${secKey}:`, sectionTitles[secKey] || '');
            if (newTitle) {
        ...
        function addSubSection(secKey) {
            if (!isAdmin) return;
            const title = prompt(`Nhập tên Nhóm Con mới cho Mục ${secKey}:`);
            if (!title) return;

            const code = prompt('Nhập mã La Mã (ví dụ: I, II, III...):', 'I');
            const subId = 'sub_' + Date.now();

            kpiDataList.push({
                id: subId,
                section: secKey,
                code: code ? code.trim() : 'I',
                title: title.trim(),
                maxScore: 10,
                items: [
                    { id: 'item_' + Date.now(), title: 'Chỉ tiêu mẫu 1', criteria: 'Yêu cầu cụ thể...', maxScore: 10, selfScore: 10, adminScore: 10 }
                ]
            });
            renderKPITable();
        }
        ...
        function deleteSubSection(subId) {
            if (!isAdmin) return;
            if (confirm('Bạn có chắc chắn muốn xóa nhóm con này và tất cả chỉ tiêu bên trong?')) {
        ...
        function addItemToSubSection(subId) {
            if (!isAdmin) return;
            const title = prompt('Nhập tên chỉ tiêu đánh giá mới:');
            if (!title) return;
        ...
        function deleteKPIItem(itemId) {
            if (!isAdmin) return;
            if (confirm('Bạn có chắc muốn xóa chỉ tiêu đánh giá này?')) {
                kpiDataList.forEach(sub => {
                    if (sub.items) {
        ...
        function exportKPIWord() {
            ...
            // Xử lý lấy Chức vụ dựa trên kpiType của user
            const targetUserInfo = registeredUsers.find(u => u.username === kpiTargetUser);
            const userKpiType = (targetUserInfo && targetUserInfo.kpiType) ? targetUserInfo.kpiType : 'staff';
            ...
            // Xử lý Kỳ đánh giá tháng... năm 2026
            let monthStr = "";
            let yearStr = "2026";
            if (selectedKpiMonth) {
                const parts = selectedKpiMonth.split('-');
                if (parts.length === 2) {
                    monthStr = parseInt(parts[1], 10);
                    yearStr = parts[0];
                }
            }

            // --- BẢNG 1: BẢNG KPI CHẤM ĐIỂM ---
            const kpiTableRows = [
                new TableRow({
                    children: [
                        new TableCell({
                        ...
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "Nội Dung Đánh Giá", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                            width: { size: 52, type: WidthType.PERCENTAGE }
                        }),
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "Điểm tối đa", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                            width: { size: 13, type: WidthType.PERCENTAGE }
                        }),
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "Điểm tự chấm", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                            width: { size: 13, type: WidthType.PERCENTAGE }
                        }),
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "Điểm đánh giá", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
            ...
            Object.keys(sectionGroups).forEach(secKey => {
                ...
                // Hàng Tiêu đề Mục lớn (A, B, C...)
                kpiTableRows.push(
                    new TableRow({
                        children: [
                            new TableCell({
                                children: [new Paragraph({ children: [new TextRun({ text: secKey, bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                width: { size: 8, type: WidthType.PERCENTAGE }
                            }),
                            new TableCell({
                                children: [new Paragraph({ children: [new TextRun({ text: secTitle.toUpperCase(), bold: true, font: "Times New Roman" })] })],
                                width: { size: 52, type: WidthType.PERCENTAGE }
                            }),
                            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "", font: "Times New Roman" })] })], width: { size: 13, type: WidthType.PERCENTAGE } }),
                            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "", font: "Times New Roman" })] })], width: { size: 13, type: WidthType.PERCENTAGE } }),
                            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "", font: "Times New Roman" })] })], width: { size: 14, type: WidthType.PERCENTAGE } })
                        ]
                    })
                );
                ...
                subList.forEach(sub => {
                    ...
                    totalSelfSum += subSelfTotal;
                    totalAdminSum += subAdminTotal;
                    ...
                    kpiTableRows.push(
                        new TableRow({
                            children: [
                                new TableCell({
                                    children: [new Paragraph({ children: [new TextRun({ text: sub.code || "", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                    width: { size: 8, type: WidthType.PERCENTAGE }
                                }),
                                new TableCell({
                                    children: [new Paragraph({ children: [new TextRun({ text: sub.title || "", bold: true, font: "Times New Roman" })] })],
                                    width: { size: 52, type: WidthType.PERCENTAGE }
                                }),
                                new TableCell({
                                    children: [new Paragraph({ children: [new TextRun({ text: String(sub.maxScore || ""), bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                    width: { size: 13, type: WidthType.PERCENTAGE }
                                }),
                                new TableCell({
                                    children: [new Paragraph({ children: [new TextRun({ text: subSelfTotal.toFixed(1), bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                    width: { size: 13, type: WidthType.PERCENTAGE }
                                }),
                                new TableCell({
                                    children: [new Paragraph({ children: [new TextRun({ text: subAdminTotal.toFixed(1), bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                    width: { size: 14, type: WidthType.PERCENTAGE }
                                })
                            ]
                        })
                        ...
                        sub.items.forEach((item, itemIdx) => {
                            kpiTableRows.push(
                                new TableRow({
                                    children: [
                                        new TableCell({
                                            children: [new Paragraph({ children: [new TextRun({ text: String(itemIdx + 1), font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                            width: { size: 8, type: WidthType.PERCENTAGE }
                                        }),
                                        new TableCell({
                                            children: [new Paragraph({ children: [new TextRun({ text: item.title || "", font: "Times New Roman" })] })],
                                            width: { size: 52, type: WidthType.PERCENTAGE }
                                        }),
                                        new TableCell({
                                            children: [new Paragraph({ children: [new TextRun({ text: String(item.maxScore || ""), font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                            width: { size: 13, type: WidthType.PERCENTAGE }
                                        }),
                                        new TableCell({
                                            children: [new Paragraph({ children: [new TextRun({ text: String(item.selfScore || ""), font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                            width: { size: 13, type: WidthType.PERCENTAGE }
                                        }),
                                        new TableCell({
                                            children: [new Paragraph({ children: [new TextRun({ text: String(item.adminScore || ""), font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                                            width: { size: 14, type: WidthType.PERCENTAGE }
                                        })
                                    ]
                                })
            ...
            kpiTableRows.push(
                new TableRow({
                    children: [
                        ...
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "Tổng điểm đánh giá", bold: true, font: "Times New Roman" })] })],
                            width: { size: 52, type: WidthType.PERCENTAGE }
                        }),
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "100", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                            width: { size: 13, type: WidthType.PERCENTAGE }
                        }),
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: totalSelfSum.toFixed(1), bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                            width: { size: 13, type: WidthType.PERCENTAGE }
                        }),
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: totalAdminSum.toFixed(1), bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                            width: { size: 14, type: WidthType.PERCENTAGE }
                        })
                    ]
                })
            ...
            // --- BẢNG 2: BẢNG XẾP LOẠI ---
            let rank1Text = "", rank2Text = "", rank3Text = "", rank4Text = "";
            let rankingResultText = "Chưa xếp loại";

            if (totalAdminSum >= 90) { rank1Text = "X"; rankingResultText = "Hoàn thành xuất sắc nhiệm vụ"; }
            else if (totalAdminSum >= 75) { rank2Text = "X"; rankingResultText = "Hoàn thành tốt nhiệm vụ"; }
            else if (totalAdminSum >= 50) { rank3Text = "X"; rankingResultText = "Hoàn thành nhiệm vụ"; }
            else { rank4Text = "X"; rankingResultText = "Không hoàn thành nhiệm vụ"; }
            ...
            const rankingTableRows = [
                new TableRow({
                    children: [
                        ...
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "Mức xếp loại", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                            width: { size: 40, type: WidthType.PERCENTAGE }
                        }),
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "Mức điểm", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                            width: { size: 30, type: WidthType.PERCENTAGE }
                        }),
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: "Đạt được", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })],
                ...
                new TableRow({
                    children: [
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "1", font: "Times New Roman" })], alignment: AlignmentType.CENTER })], width: { size: 10, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Hoàn thành xuất sắc nhiệm vụ", bold: true, font: "Times New Roman" })] })], width: { size: 40, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Từ 90 điểm trở lên", font: "Times New Roman" })] })], width: { size: 30, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: rank1Text, bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })], width: { size: 20, type: WidthType.PERCENTAGE } })
                    ]
                }),
                new TableRow({
                    children: [
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2", font: "Times New Roman" })], alignment: AlignmentType.CENTER })], width: { size: 10, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Hoàn thành tốt nhiệm vụ", bold: true, font: "Times New Roman" })] })], width: { size: 40, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Từ 75 đến dưới 90 điểm", font: "Times New Roman" })] })], width: { size: 30, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: rank2Text, bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })], width: { size: 20, type: WidthType.PERCENTAGE } })
                    ]
                }),
                new TableRow({
                    children: [
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "3", font: "Times New Roman" })], alignment: AlignmentType.CENTER })], width: { size: 10, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Hoàn thành nhiệm vụ", bold: true, font: "Times New Roman" })] })], width: { size: 40, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Từ 50 đến dưới 75 điểm", font: "Times New Roman" })] })], width: { size: 30, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: rank3Text, bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })], width: { size: 20, type: WidthType.PERCENTAGE } })
                    ]
                }),
                new TableRow({
                    children: [
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "4", font: "Times New Roman" })], alignment: AlignmentType.CENTER })], width: { size: 10, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Không hoàn thành nhiệm vụ", bold: true, font: "Times New Roman" })] })], width: { size: 40, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Dưới 50 điểm", font: "Times New Roman" })] })], width: { size: 30, type: WidthType.PERCENTAGE } }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: rank4Text, bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })], width: { size: 20, type: WidthType.PERCENTAGE } })
                    ]
                })
            ];

            // Bảng ẩn viền để căn Header góc trên trái / góc trên phải
                ...
                rows: [
                    new TableRow({
                        children: [
                            new TableCell({
                                children: [
                                    new Paragraph({ children: [new TextRun({ text: "TRUNG TÂM KSBT BẮC NINH", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER }),
                                    new Paragraph({ children: [new TextRun({ text: "KHOA HÓA LÝ", bold: true, underline: {}, font: "Times New Roman" })], alignment: AlignmentType.CENTER })
                                ],
                                width: { size: 45, type: WidthType.PERCENTAGE },
                            ...
                            new TableCell({
                                children: [
                                    new Paragraph({ children: [new TextRun({ text: "CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER }),
                                    new Paragraph({ children: [new TextRun({ text: "Độc lập - Tự do - Hạnh phúc", bold: true, underline: {}, font: "Times New Roman" })], alignment: AlignmentType.CENTER })
                                ],
            ...
            // Bảng ẩn viền để căn chữ ký ở lệch bên phải
            const footerSignTable = new Table({
                rows: [
                    new TableRow({
                        children: [
                            new TableCell({
                            ...
                            new TableCell({
                                children: [
                                    new Paragraph({ children: [new TextRun({ text: `Bắc Ninh, ngày....tháng....năm ${yearStr}`, italics: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER }),
                                    new Paragraph({ children: [new TextRun({ text: "XÁC NHẬN CỦA LÃNH ĐẠO KHOA, TRƯỞNG KHOA", bold: true, font: "Times New Roman" })], alignment: AlignmentType.CENTER })
                                ],
                                width: { size: 55, type: WidthType.PERCENTAGE },
                                borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } }
                            })
            ...
            const doc = new Document({
                sections: [{
                    ...
                    children: [
                        ...
                        new Paragraph({
                            children: [new TextRun({ text: `Họ và Tên: ${kpiTargetUser.toUpperCase()}`, font: "Times New Roman", size: 24 })],
                            spacing: { after: 100 }
                        }),
                        new Paragraph({
                            children: [new TextRun({ text: `Chức vụ: ${chucVuText}`, font: "Times New Roman", size: 24 })],
                            spacing: { after: 200 }
                        }),
                        new Paragraph({
                            children: [new TextRun({ text: "PHIẾU THEO DÕI ĐÁNH GIÁ VIÊN CHỨC", bold: true, size: 28, font: "Times New Roman" })],
                            alignment: AlignmentType.CENTER,
                            spacing: { after: 100 }
                        }),
                        new Paragraph({
                            children: [new TextRun({ text: `(Kỳ đánh giá tháng ${monthStr} năm ${yearStr})`, italics: true, size: 24, font: "Times New Roman" })],
                            alignment: AlignmentType.CENTER,
                            spacing: { after: 300 }
                        }),
                        new Table({
                        ...
                        new Paragraph({
                            children: [new TextRun({ text: `Kết quả xếp loại: ${rankingResultText}`, bold: true, font: "Times New Roman", size: 24 })],
